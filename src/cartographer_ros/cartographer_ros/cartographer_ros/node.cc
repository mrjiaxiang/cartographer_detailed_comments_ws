...
Node::Node(
    ...
    service_servers_.push_back(
        node_handle_.advertiseService(kGetTrajectoryStatesServiceName,
...
void Node::PublishLocalTrajectoryData(const ::ros::TimerEvent &timer_event) {
    ...
    for (const auto &entry : map_builder_bridge_.GetLocalTrajectoryData()) {
        ...
        // high frequency, and republishing it would be computationally
        // wasteful. 如果当前状态的时间与extrapolator的lastposetime不等
        if (trajectory_data.local_slam_data->time !=
            ...
            if (scan_matched_point_cloud_publisher_.getNumSubscribers() > 0) {
                ...
                point_cloud.reserve(trajectory_data.local_slam_data
        ...
        // published poses to advance. If we already know a newer pose, we use
        // its time instead. Since tf knows how to interpolate, providing newer
        ...
        // Due to 2020-07 changes to geometry2, tf buffer will issue warnings
        // for repeated transforms with the same timestamp.
        ...
        if (trajectory_data.published_to_tracking != nullptr) {
            if (node_options_.publish_to_tf) {
                ...
                // cartographer不需要提供odom坐标系,则发布 map_frame ->
                // published_frame 的tf
                else {
                    stamped_transform.header.frame_id = node_options_.map_frame;
                    stamped_transform.child_frame_id =
...
Node::ComputeExpectedSensorIds(const TrajectoryOptions &options) const {
    ...
    */
    ...
    // Subscribe to all laser scan, multi echo laser scan, and point cloud
    // topics.
    ...
    // For 2D SLAM, subscribe to the IMU if we expect it. For 3D SLAM, the IMU
    // is required. 3d slam必须有imu, 2d可有可无, imu的topic的个数只能有一个
    if (node_options_.map_builder_options.use_trajectory_builder_3d() ||
...
void Node::LaunchSubscribers(const TrajectoryOptions &options,
                             const int trajectory_id) {
    // laser_scan 的订阅与注册回调函数, 多个laser_scan 的topic
    // 共用同一个回调函数
    for (const std::string &topic :
         ComputeRepeatedTopicNames(kLaserScanTopic, options.num_laser_scans)) {
    ...
    // For 2D SLAM, subscribe to the IMU if we expect it. For 3D SLAM, the IMU
    // is required. imu 的订阅与注册回调函数,只有一个imu的topic
    if (node_options_.map_builder_options.use_trajectory_builder_3d() ||
        (node_options_.map_builder_options.use_trajectory_builder_2d() &&
         options.trajectory_builder_options.trajectory_builder_2d_options()
             .use_imu_data())) {
...
bool Node::HandleStartTrajectory(
    ...
    if (request.use_initial_pose) {
        ...
        if (!pose.IsValid()) {
            response.status.message = "Invalid pose argument. Orientation "
        ...
        // Check if the requested trajectory for the relative initial pose
        // exists. 检查 initial_pose 对应的轨迹id是否存在
        response.status = TrajectoryStateToStatus(
    ...
    for (const auto &entry : subscribers_) {
        ...
        for (const auto &subscriber : entry.second) {
            ...
            std::string resolved_topic =
                node_handle_.resolveName(subscriber.topic);
    ...
    for (; node_it != end_it; ++node_it) {
        ...
        if (trajectory_node.constant_data != nullptr) {
            ...
            for (const RangefinderPoint &point :
                ...
                RangefinderPoint range_finder_point =
                    global_pose.cast<float>() * point;
                node_point_cloud->push_back(
                    pcl::PointXYZ(range_finder_point.position.x(),
