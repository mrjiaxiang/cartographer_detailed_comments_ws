...
visualization_msgs::Marker CreateTrajectoryMarker(const int trajectory_id,
    ...
    marker.id = 0;
    // note: Marker::LINE_STRIP 它会在每两个连续的点之间画一条线 eg: 0-1, 1-2,
    // 2-3
...
MapBuilderBridge::MapBuilderBridge(
    ...
    : node_options_(node_options), map_builder_(std::move(map_builder)),
...
void MapBuilderBridge::HandleSubmapQuery(
    ...
    for (const auto &texture_proto : response_proto.textures()) {
        ...
        // 对引用的变量进行赋值
        texture.cells.insert(texture.cells.begin(),
...
MapBuilderBridge::GetTrajectoryStates() {
    ...
    for (const auto &sensor_bridge : sensor_bridges_) {
        trajectory_states.insert(std::make_pair(
            sensor_bridge.first, ::cartographer::mapping::PoseGraphInterface::
...
visualization_msgs::MarkerArray MapBuilderBridge::GetTrajectoryNodeList() {
    ...
    for (const auto &constraint : constraints) {
        ...
        if (constraint.tag == cartographer::mapping::PoseGraphInterface::
            ...
            if (constraint.node_id.trajectory_id ==
                ...
                trajectory_to_last_inter_submap_constrained_node
                    [constraint.node_id.trajectory_id] = std::max(
                        trajectory_to_last_inter_submap_constrained_node.at(
    ...
    for (const int trajectory_id : node_poses.trajectory_ids()) {
        ...
        int last_inter_trajectory_constrained_node = std::max(
            node_poses.trajectory(trajectory_id).begin()->id.node_index,
            trajectory_to_last_inter_trajectory_constrained_node.at(
                trajectory_id));
        ...
        for (const auto &node_id_data : node_poses.trajectory(trajectory_id)) {
            ...
            if (marker.points.size() == 16384) {
                PushAndResetLineMarker(&marker, &trajectory_node_list.markers);
                // Push back the last point, so the two markers appear
                // connected.
                marker.points.push_back(node_point);
            }
        } // end for
...
visualization_msgs::MarkerArray MapBuilderBridge::GetConstraintList() {
    ...
    for (const auto &constraint : constraints) {
        ...
        // 根据不同情况,将constraint_marker与residual_marker
        // 指到到不同的maker类型上
        ...
        if (constraint.tag == cartographer::mapping::PoseGraphInterface::
            ...
            // Color mapping for submaps of various trajectories - add
            // trajectory id to ensure different starting colors. Also add a
            // fixed offset of 25 to avoid having identical colors as
            // trajectories.
