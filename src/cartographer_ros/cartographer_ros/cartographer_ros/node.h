...
class Node {
  public:
    ...
    // Finishes a single given trajectory. Returns false if the trajectory did
    // not exist or was already finished.
    bool FinishTrajectory(int trajectory_id);

    // Runs final optimization. All trajectories have to be finished when
    // calling.
    ...
    void
    HandlePointCloud2Message(int trajectory_id, const std::string &sensor_id,
  ...
  private:
    struct Subscriber {
        ...
        // std::string
        // it was given in its constructor. Since we rely on the topic name as
        // the unique identifier of a subscriber, we remember it ourselves.
        std::string topic;
    };

    bool
    HandleSubmapQuery(cartographer_ros_msgs::SubmapQuery::Request &request,
    ...
    bool
    HandleWriteState(cartographer_ros_msgs::WriteState::Request &request,
                     cartographer_ros_msgs::WriteState::Response &response);
    bool HandleGetTrajectoryStates(
        ::cartographer_ros_msgs::GetTrajectoryStates::Request &request,
        ::cartographer_ros_msgs::GetTrajectoryStates::Response &response);
    bool
    HandleReadMetrics(cartographer_ros_msgs::ReadMetrics::Request &request,
    ...
    // Helper function for service handlers that need to check trajectory
    // states.
    ...
    // Analysis（线程安全分析）中定义的属性 GUARDED_BY是数据成员的属性,
    // 该属性声明数据成员受给定功能保护. 对数据的读操作需要共享访问,
    // 而写操作则需要互斥访问. 官方介绍文档:
    ...
    // simulation time is standing still. This prevents overflowing the
    // transform listener buffer by publishing the same transforms over and over
    // again.
