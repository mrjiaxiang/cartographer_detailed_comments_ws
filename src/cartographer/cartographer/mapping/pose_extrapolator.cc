...
void PoseExtrapolator::AddOdometryData(
    ...
    // 根据位姿队列中最后一个位姿 乘以
    // 上次添加位姿时的姿态预测到time时刻的姿态变化量 得到预测的
    // 最新里程计数据时刻 tracking frame 在 local 坐标系下的姿态
...
void PoseExtrapolator::AdvanceImuTracker(const common::Time time,
    ...
    if (imu_data_.empty() || time < imu_data_.front().time) {
        ...
        // stability. 在time之前没有IMU数据, 因此我们推进ImuTracker,
...
PoseExtrapolator::ExtrapolatePosesWithGravity(
    ...
    // 个元素的迭代器,而不改变输入迭代器的值 默认 n 为1,当 n 为正数时,
    // 其返回的迭代器将位于 it 左侧； 反之, 当 n 为负数时, 其返回的迭代器位于 it
    // 右侧
