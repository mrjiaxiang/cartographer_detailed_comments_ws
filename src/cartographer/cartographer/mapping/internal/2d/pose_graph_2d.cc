...
std::vector<SubmapId> PoseGraph2D::InitializeGlobalSubmapPoses(
    ...
    if (insertion_submaps.size() == 1) {
        ...
        // 如果判断指定id的submap_data的size为0,
        // 这条轨迹上还没有添加submap的pose
    ...
    // end_it是最后一个元素的下一个位置,
    // 所以它之前的一个submap的id就是submap_data中的最后一个元素 注意,
    // 这里的last_submap_id 是 optimization_problem_->submap_data() 中的
    ...
    if (data_.submap_data.at(last_submap_id).submap ==
        ...
        // 解算新的submap的global pose,
        // 插入到OptimizationProblem2D::submap_data_中
        optimization_problem_->AddSubmap(
            trajectory_id,
            // first_submap_pose *
            // constraints::ComputeSubmapPose(*insertion_submaps[0]).inverse() =
            // globla指向local的坐标变换 globla指向local的坐标变换 *
            // 第二个子图原点在local下的坐标 = 第二个子图原点在global下的坐标
            first_submap_pose *
...
WorkItem::Result PoseGraph2D::ComputeConstraintsForNode(
    ...
    if (options_.optimize_every_n_nodes() >
            0 && // param: optimize_every_n_nodes
        num_nodes_since_last_loop_closure_ >
...
void PoseGraph2D::DrainWorkQueue() {
    ...
    // 退出循环后, 首先等待计算约束中的任务执行完,
    // 再执行HandleWorkQueue,进行优化
...
void PoseGraph2D::AddSubmapFromProto(
    ...
    AddWorkItem([this, submap_id,
                 global_submap_pose_2d]() LOCKS_EXCLUDED(mutex_) {
        absl::MutexLock locker(&mutex_);
...
void PoseGraph2D::SetTrajectoryDataFromProto(
    ...
    if (data.has_fixed_frame_origin_in_map()) {
        ...
        AddWorkItem(
            [this, trajectory_id, trajectory_data]() LOCKS_EXCLUDED(mutex_) {
                absl::MutexLock locker(&mutex_);
                if (CanAddWorkItemModifying(trajectory_id)) {
...
void PoseGraph2D::RunOptimization() {
    ...
    // mutex before Solve to avoid blocking foreground processing. Solve
    // 比较耗时, 所以在执行 Solve 之前不要加互斥锁, 以免阻塞其他的任务处理
    // landmark直接参与优化问题
...
void PoseGraph2D::TrimmingHandle::TrimSubmap(const SubmapId &submap_id) {
    ...
    // save memory.
    {
        ...
        // Step: 2 删除与nodes_to_remove中节点相关联的约束,
        // 并对submap_id进行标记
        for (const Constraint &constraint : parent_->data_.constraints) {
        ...
        for (const Constraint &constraint : parent_->data_.constraints) {
            ...
            // 就把这个子图id从other_submap_ids_losing_constraints中删除,
            // 可以留着
